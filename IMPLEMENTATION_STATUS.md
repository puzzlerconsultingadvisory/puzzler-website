...
- **Section order (2026-09-09, founder request).** Build It to Hold ("Who we serve and how we help") now follows The Operator Gap directly, ahead of The Puzzler Method; the primary nav lists them in the same order. It sits on Pale Gray so it reads as its own band after the white Operator Gap.
- **Contact details capture (2026-09-09, founder request).** Every starting-point result now carries a collapsible "Prefer we reach out? Leave your details." form (name*, organization, email*, phone, note), and the Fit Call section carries the same form beneath Book a Fit Call. The site has no form backend and none was added: submitting opens the visitor's email app addressed to the approved inquiry address with the fields and the chosen starting point in the body (`mailto:`), so nothing is sent to or stored by the website and the Privacy Notice is unchanged. Limitation: a visitor without a configured email app gets no delivery. If you want submissions to arrive without depending on the visitor's mail client, choose a form service (or use Calendly's booking questions) and I will wire it and update the Privacy Notice accordingly.
- **Digital storytelling as a service option (2026-09-09, founder request).** Added as a seventh starting point ("We need to tell our story" → *Digital Storytelling*, CTA "Start the Story") and as an "Also available" note beneath the five practices linking to Pieces in Motion. The five-practice set is unchanged. All copy is DRAFT (marked in `index.html`): it is drawn only from the Pieces in Motion labels already on the site (digital storytelling, functional audiovisual work) and needs the founder's confirmation of deliverables, scope, and which practices it should map to (currently Funding Strategy, Program Effectiveness and Accountability, Leadership Development).
